...
export const PublicCampaignSignupPage: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (result.outcome === 'campaign_disabled_no_existing') {
      setError('Los nuevos registros están pausados para esta campaña. Si ya tienes una tarjeta en curso, ingresa el mismo correo o número móvil que usaste antes.');
      return;
    }
  ...
  if (!context) {
    return (
      <div className="h-screen flex items-center justify-center px-6 text-center text-muted-foreground">
        El enlace de registro de campaña es inválido.
      </div>
    );
  }
      ...
      <div className="mx-auto w-full max-w-xl">
        <section className="rounded-[2rem] border border-black/[0.08] bg-white p-6 shadow-[0_24px_64px_-38px_rgba(0,0,0,0.35)] sm:p-8">
          ...
          <p className="mt-3 text-[0.98rem] leading-7 text-[#4f5258]">
            Únete a <span className="font-semibold text-[#1d1d1f]">{context.campaign.name}</span> para comenzar a acumular sellos.
          </p>

          {disabled && (
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-800">
              Los nuevos registros están pausados para esta campaña. Si ya tienes una tarjeta en curso, ingresa el mismo correo o móvil para continuar.
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-5">
            ...
            <div className="grid gap-2">
              <Label htmlFor="email" className="text-sm font-medium text-[#1d1d1f]">
                Correo electrónico <span className="text-[#6e6e73]">(Opcional)</span>
              </Label>
              <Input
                id="email"
                value={email}
                ...
                className="h-12 rounded-xl border-black/10 text-[#1d1d1f] placeholder:text-[#8f9197]"
              />
              <p className="text-xs leading-5 text-[#6e6e73]">Usado para recuperar tu tarjeta y notificaciones de premios.</p>
            </div>

            <div className="grid gap-2">
              <Label htmlFor="mobile" className="text-sm font-medium text-[#1d1d1f]">
                Número de móvil <span className="text-[#6e6e73]">(Opcional)</span>
              </Label>
              <Input
                id="mobile"
                value={mobile}
